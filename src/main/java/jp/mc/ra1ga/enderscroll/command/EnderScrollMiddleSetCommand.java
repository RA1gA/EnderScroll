...
public class EnderScrollMiddleSetCommand implements SubCommandable {
	...
	public boolean run(CommandSender sender, String[] args, JavaPlugin plugin) {
		try{
			...
			ScrollLocation sloc = new ScrollLocation(locName, world, x, y, z, 0, 0);
	...
	public List<String> getHelp() {
		return Arrays.asList(
				ChatColor.YELLOW + "/enderscroll set <LocationName> world x y z",
				ChatColor.YELLOW + "<LocationName>で指定の座標を保存する"
		);
	}
